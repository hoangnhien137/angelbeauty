...
*/
/*
if(!empty($_GET['import']) && ($_GET['import'] == 'post')) {
    ...
    // Dich vu 95
        // Cham soc da mat 98
        $aMaps[] = array(
            'blog_master_categories_id' => 7,
            'post_category' => array(95,98)
            );
        // Cham soc body 97
        $aMaps[] = array(
            'blog_master_categories_id' => 6,
            'post_category' => array(95,97)
            );
        // Phun theu tham my 9
        $aMaps[] = array(
            'blog_master_categories_id' => 9,
            'post_category' => array(95,96)
            );
        // Triet long vinh vien 8
        $aMaps[] = array(
            'blog_master_categories_id' => 8,
            'post_category' => array(95,99)
            );
    // Tin tuc new=66
        // Khuyen mai old=18, new = 92
        $aMaps[] = array(
            'blog_master_categories_id' => 18,
            'post_category' => array(66,92)
            );
        // Lam dep old=17, new = 89
        $aMaps[] = array(
            'blog_master_categories_id' => 17,
            'post_category' => array(66,89)
            );
        // Thoi trang old=16, new = 90
        $aMaps[] = array(
            'blog_master_categories_id' => 16,
            'post_category' => array(66,90)
            );
        // Sao&Angel old=46, new=94
        $aMaps[] = array(
            'blog_master_categories_id' => 46,
            'post_category' => array(66,94)
            );
        // Bao chi old=47, new=91
        $aMaps[] = array(
            'blog_master_categories_id' => 47,
            'post_category' => array(66,91)
            );
        // Tuyen dung old=44, new=93
        $aMaps[] = array(
            'blog_master_categories_id' => 44,
            'post_category' => array(66,93)
            );
    // Hoat dong 67
        // Tong giam doc old=22,new=70
        $aMaps[] = array(
            'blog_master_categories_id' => 22,
            'post_category' => array(67,70)
            );
        // Truong trinh tai tro old=48, new=69
        $aMaps[] = array(
            'blog_master_categories_id' => 48,
            'post_category' => array(67,69)
            );
        // Giai thuong old=20,new=68
        $aMaps[] = array(
            'blog_master_categories_id' => 20,
            'post_category' => array(67,68)
            );


    $mydb = new wpdb('root','','old_angelbeauty','localhost');
    set_time_limit(-1);
    foreach( $aMaps as $aMap ){
        $aBlogs = $mydb->get_results("SELECT blog_id,blog_image FROM blog WHERE blog_master_categories_id = " .$aMap['blog_master_categories_id']);
        if( !empty($aBlogs) ){
            foreach( $aBlogs as $oBlog ){
                $blog = $mydb->get_results("SELECT blog_title, blog_content FROM blog_content WHERE blog_id = " . $oBlog->blog_id);
                foreach ( $blog as $blog_row ) {
                    $aNewBlog = array(
                        'post_title' => $blog_row->blog_title,
                        'post_content' => $blog_row->blog_content,
                        'post_type' => 'post',
                        'post_status'   => 'publish',
                        'post_author'   => 1,
                        'post_category' => $aMap['post_category']
                    );
                    $iNewId = wp_insert_post( $aNewBlog );

                    add_post_meta($iNewId, '_link_redirect_post', $oBlog->blog_id);
                    add_post_meta($iNewId, '_custom_thumbnail', $upload_dir['baseurl'] . "/blog/" . $oBlog->blog_image);
                }
            }
...
*/
/*
if(!empty($_GET['import']) && ($_GET['import'] == 'ykkh')) {
    $upload_dir = wp_upload_dir();
...
*/
/*
if(!empty($_GET['import']) && ($_GET['import'] == 'faq')) {
	$mydb = new wpdb('root','','old_angelbeauty','localhost');
    set_time_limit(-1);
	$aFaqs = $mydb->get_results("SELECT faqs_title,faqs_content,faqs_date,faqs_name,faqs_email FROM faqs_content ORDER BY faqs_date");
	$i = 0;
	foreach ( $aFaqs as $oFaq ) {
		$aNewFaq = array(
			'post_title' => $oFaq->faqs_name,
			'post_content' => $oFaq->faqs_title,
			'post_type' => 'question-and-answer',
			'post_status'   => 'publish',
			'post_author'   => 1,
			'post_date'      =>  date('Y-m-d H:i:s', $oFaq->faqs_date),//[ Y-m-d H:i:s ]
			'post_excerpt'   => $oFaq->faqs_email, //[ <an excerpt> ]
		);
		
		$iNewId = wp_insert_post( $aNewFaq );
		add_post_meta($iNewId, '_answer', $oFaq->faqs_content);
		//add_post_meta($iNewId, '_custom_thumbnail', $upload_dir['baseurl'] . "/blog/" . $oBlog->blog_image);
		//$i++;
		//if ($i == 1) break;
	}
...
*/
/*
if(!empty($_GET['import']) && ($_GET['import'] == 'post_2')) {
    $upload_dir = wp_upload_dir();
    
    $aMaps = array();
    // Cong nghe may moc new=79
		// Cong nghe moi old=50,new=85
		$aMaps[] = array(
            'blog_master_categories_id' => 50,
            'post_category' => array(79,85)
            );
		// RF Hien dai old=10
		$aMaps[] = array(
            'blog_master_categories_id' => 10,
            'post_category' => array(79,84)
            );
		// Dien tro SH old=37
		$aMaps[] = array(
            'blog_master_categories_id' => 37,
            'post_category' => array(79,87)
            );
		// Phan tich da old=14
		$aMaps[] = array(
            'blog_master_categories_id' => 14,
            'post_category' => array(79,80)
            );
		// Laser old=13
		$aMaps[] = array(
            'blog_master_categories_id' => 13,
            'post_category' => array(79,82)
            );
		// Sieu am old=36
		$aMaps[] = array(
            'blog_master_categories_id' => 36,
            'post_category' => array(79,88)
            );
		// Song dien tu old=38
		$aMaps[] = array(
            'blog_master_categories_id' => 38,
            'post_category' => array(79,81)
            );
		// Anh sang old=11
		$aMaps[] = array(
            'blog_master_categories_id' => 11,
            'post_category' => array(79,83)
            );
		// Oxyen old=35
		$aMaps[] = array(
            'blog_master_categories_id' => 35,
            'post_category' => array(79,86)
            );
...
*/
/*
if(!empty($_GET['import']) && ($_GET['import'] == 'clips')) {
    $upload_dir = wp_upload_dir();
    
    $mydb = new wpdb('root','','old_angelbeauty','localhost');
    set_time_limit(-1);
	$aClips = $mydb->get_results("SELECT clips_id, clips_filename, clips_url, clips_title FROM clips");
	foreach ( $aClips as $aClip ) {
		$aNewClip = array(
			'post_title' => $aClip->clips_title,
			'post_content' => $aClip->clips_url,
			'post_type' => 'post',
			'post_status'   => 'publish',
			'post_author'   => 1,
			'post_category' => array(72),
		);
		$iNewId = wp_insert_post( $aNewClip );

		add_post_meta($iNewId, '_link_redirect_post', $aClip->clips_id);
		add_post_meta($iNewId, '_custom_thumbnail', $upload_dir['baseurl'] . "/clip/" . $aClip->clips_filename);
	}
}
*/
/*
if(!empty($_GET['import']) && ($_GET['import'] == 'images')) {
    $upload_dir = wp_upload_dir();
    
	//$aAlbums = array(55,54,53,52,49,48,50,51,47,45,25,44,32,27);
	//$aAlbums = array(52,49,48,50,51,47,45,25,44,32,27);
    $aAlbums = array(27,32,44,25,45,47,51,50,48,49,52);
	$mydb = new wpdb('root','','old_angelbeauty','localhost');
    set_time_limit(-1);
	//$i = 0;
	foreach( $aAlbums as $iAlbumID ){
		$aAlbum_content = $mydb->get_results("SELECT bbs_title FROM bbs_content WHERE bbs_id = " . $iAlbumID);
		$sAlbumTitle = '';
		foreach( $aAlbum_content as $oCurAlbum ) {
				$sAlbumTitle = $oCurAlbum->bbs_title;
		}
		
		$aAlbum_files = $mydb->get_results("SELECT bbs_file_name FROM bbs_file WHERE bbs_id = " . $iAlbumID);
		
		$aOldAlbum = array();
		foreach ( $aAlbum_files as $aAlbum_file ) {
			$aOldAlbum[] = $upload_dir['baseurl'] . "/album/" . $aAlbum_file->bbs_file_name;
		}
			$aNewAlbum = array(
				'post_title' => $sAlbumTitle,
				'post_content' => 'Angel Beauty Album',
				'post_type' => 'post',
				'post_status'   => 'publish',
				'post_author'   => 1,
				'post_category' => array(71),
			);
			$iNewId = wp_insert_post( $aNewAlbum );

			add_post_meta($iNewId, '_link_redirect_post', $iAlbumID);
			add_post_meta($iNewId, '_custom_thumbnail', $upload_dir['baseurl'] . "/album_thumb/" .$iAlbumID. ".jpg");
			add_post_meta($iNewId, '_old_album', json_encode($aOldAlbum) );
		//$i++;
		//if ($i==2) break;
	}
...
if(!empty($_GET['import']) && ($_GET['import'] == 'spa')) {
    $upload_dir = wp_upload_dir();
    
    $aMaps = array();
    	$aMaps[] = array(
            'blog_master_categories_id' => 49,
            'post_category' => array(100)
            );
